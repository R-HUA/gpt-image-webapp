...
interface AppState {
  ...
  setServerImageBatchMode: (enabled: boolean) => void
...
async function fetchBackendResultImage(record: { outputUrl: string }) {
  if (record.outputUrl.startsWith('data:')) return record.outputUrl
...
export async function submitTask(options: { allowFullMask?: boolean; useCurrentApiProfileWhenReusedMissing?: boolean } = {}) {
  ...
  const serverImagePath = serverImageBatchMode ? String(settings.adminServerImagePath || '').trim() : ''
...
async function executeTask(taskId: string) {
  ...
  const maybeTask = useStore.getState().tasks.find((t) => t.id === taskId)
  if (!maybeTask) return
...

  {
    ...
    try {
          ...
          for (const record of backendJob.progress.completedRecords) {
            ...
            if (sibling && sibling.status !== 'done' && sibling.status !== 'cancelled') {
              try {
                ...
                // Final synchronization handles transient image fetch failures.
              }
            }
          }
        }
      ...
      if (backendJob.status === 'cancelled') {
        ...
        for (const target of targets) {
          if (isTerminalTaskStatus(target.status) && target.status !== 'cancelled') continue
          updateTaskInStore(target.id, {
            status: 'cancelled',
      ...
      if (currentTaskState.backendJobOwner && currentTaskState.batchId) {
        for (const sibling of getBatchSiblings(currentTaskState)) {
          if (sibling.status === 'done' && sibling.outputImages?.length && sibling.batchIndex && !sibling.hiddenByRetry) {
      ...
      if (resultOutputs.length === 0) throw new Error('后端任务未返回图片')
      ...
      if (latestBeforeUpdate.backendJobOwner && latestBeforeUpdate.batchId) {
        ...
        if (!maybeShowBatchCompletionToast(latestBeforeUpdate)) {
          useStore.getState().showToast(`批量任务完成 ${storedOutputs.length}/${siblings.length}`, 'success')
        }
        return
      }
